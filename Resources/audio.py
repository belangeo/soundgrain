...
class Granulator_Stream:
    def __init__(self, order, env, trans_noise, dur_noise, num_grains, amplitude, clock_func):
        self.order = order
        self.env = env
    ...
    def create_granulator(self, table, pos_rnd):
        ...
        self.granulator = Granulator(table=self.table, env=self.env, pitch=self.base_pitch*self.pitch,
                                    ...
                                    mul=self.fader*self.amplitude*self.amp
                                    ).stop()
    ...
    def boot(self, driver, chnls):
        ...
        for i in range(24):
            self.streams[i] = Granulator_Stream(i, self.env, self.trans_noise, self.dur_noise, 
                                                self.num_grains, self.amplitude, self.clock)
